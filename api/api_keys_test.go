...
func TestClient_ListApiKey(t *testing.T) {
	testCases := []struct {
		...
		{"success response", `{"code": 200, "message": "OK", "status": "success", "apikeys": ["foo", "bar"]}`, 200, &ListApiKeyResponse{LoadMasterResponse: &LoadMasterResponse{Code: 200, Message: "OK", Status: "success"}, ApiKeys: []string{"foo", "bar"}}, false},
